...
| 113 | Política de privacidad de Elena App (`/elenaapp/privacidad`) — App Store + Play Store compliant | ✅ Cerrada (2026-05-25) | Documento legal obligatorio para publicar Elena App en Apple App Store y Google Play. Conforme a Ley 1581 Colombia + GDPR + CCPA + LGPD. 19 secciones: identidad, salud (categoría especial GDPR Art. 9), hábitos, uso, técnicos, financieros, HealthKit/Google Fit, cámara, push notifications, compras in-app. Datos mapeados 1:1 al schema `users/{uid}` de SPEC-005. Tabla de mapeo datos → categorías Apple/Google incluida en la spec para completar los formularios de ambas stores. Sitemap ahora incluye también `/elenaapp/privacidad`, `/elenasupport` y `/imr/metodologia` (cierra hallazgo H5 del reporte SEO 2026-05-25). Pendiente review legal antes del launch comercial. | [SPEC-113](specs/SPEC-113-elenaapp-privacy-policy.md) |
